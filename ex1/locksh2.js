...
class Locks {
	...
	alternateRoute(wheelNum, r, deadends, target) {
		...
		const mapres =  otherWheels.map(wheelN => {
			...
			const shmres = this.addNextCombination(nextCombination, res);
			// shmres[shmres.length-1].push(nextCombination);

			// out.push([...res]);
			// console.log(`+++ my next shmres: ${JSON.stringify(shmres)}`);
			const nextWh = this.getNextWheel(wheelN, target);
			// out.concat(res.push([this.turn(nextWh, res, deadends, target)]));
			return this.turn(nextWh, shmres, deadends, target);
			// console.log(`+++ out: ${JSON.stringify(out)}`);
			// out.push([...innerRes]);
			// console.log(`+++ innerRes: ${JSON.stringify(innerRes)}; r: ${JSON.stringify(r)}; res: ${JSON.stringify(res)};`);
			// return innerRes;
		});
	...
	turnWheel(wheelNum, combination, comboWheelVal, targetWheelVal) {
